...
function stats() { let a = todayAll(), fin = a.filter(x => x.estado === 'FINALIZADA'), curso = a.filter(x => x.estado === 'EN PROCESO'); let ms = fin.reduce((s, x) => s + (new Date(x.fin) - new Date(x.inicio)), 0), hh = fin.reduce((s, x) => s + ((new Date(x.fin) - new Date(x.inicio)) / 36e5 * x.trabajadores), 0); return { a, fin, curso, ms, hh }; }
function home() { let s = stats(), mins = Math.floor(s.ms / 60000), pct = s.a.length ? Math.round(s.fin.length / s.a.length * 100) : 0; app.innerHTML = `${internalHeader('HOME', true)}<main class="page home-page"><section class="kpis home-kpis"><article class="k-blue"><b>${s.a.length}</b><small>Incidencias registradas</small></article><article class="k-orange"><b>${s.curso.length}</b><small>En proceso</small></article><article class="k-green"><b>${s.fin.length}</b><small>Finalizadas</small></article><article class="k-purple"><b>${Math.floor(mins / 60)}h ${String(mins % 60).padStart(2, '0')}m</b><small>Tiempo muerto</small></article><article class="k-cyan"><b>${s.a.reduce((q, x) => q + x.trabajadores, 0)}</b><small>Trabajadores afectados</small></article><article class="k-red"><b>${s.hh.toFixed(1)} h</b><small>Horas-hombre perdidas</small></article></section><section class="chart-card home-chart"><h3>En proceso vs. finalizadas</h3><div class="donut" style="--p:${pct}"><b>${pct}%</b></div><div class="legend"><span>● En proceso: ${s.curso.length}</span><span>● Finalizadas: ${s.fin.length}</span></div></section></main>`; }
...
function nuevo() { app.innerHTML = `${internalHeader('Nueva incidencia')}<main class="page"><form id="newForm" class="form"><label>Grupo<input value="G003" readonly></label><label>Supervisor<input value="Cesar Santisteban" readonly></label><label>Fundo<input value="Agricultor" readonly></label><label>Parcela<select id="parcela" required><option value="">Seleccionar parcela</option><option>Parcela 61</option><option>Parcela 62</option><option>Parcela 64</option></select></label><label>Área<input value="Cosecha" readonly></label><label>Fecha y hora de inicio<input id="inicio" readonly></label><label>Tipo de incidente<select id="tipo" required><option value="">Seleccionar</option><option>Retraso de material</option><option>Falla de maquinaria</option><option>Falla de personal</option><option>Retraso de transporte</option><option>Otro</option></select></label><label>Descripción<textarea id="desc" required placeholder="Describe la incidencia"></textarea></label><label>Trabajadores afectados<input value="45" readonly></label><label>Evidencia (Foto)<input id="foto" type="file" accept="image/*" capture="environment"></label><button class="primary">Guardar incidencia</button></form></main>`; inicio.value = fmt(new Date()); newForm.onsubmit = e => { e.preventDefault(); let nums = Incidencias.all().map(x => +x.id.replace(/\D/g, '')), n = (Math.max(0, ...nums) + 1), x = { id: 'INC-' + String(n).padStart(5, '0'), grupo: 'G003', supervisor: 'Cesar Santisteban', fundo: 'Agricultor', parcela: parcela.value, area: 'Cosecha', inicio: new Date().toISOString(), tipo: tipo.value, descripcion: desc.value, trabajadores: 45, estado: 'EN PROCESO', sincronizado: false, fechaRegistro: new Date().toISOString() }; Incidencias.save(x); toast('Incidencia registrada correctamente'); filtro = 'todos'; setTimeout(() => go('registro'), 450); }; }
